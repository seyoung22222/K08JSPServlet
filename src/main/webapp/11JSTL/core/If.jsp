...
속성정리
 ...
 -->
	...
	<%-- <c:set value="${param.user}" var="user_id"/>
	<c:set value=" ${param.pass}" var="user_pw"/>
	<c:set var="number" value="1234"/>
	<c:if test="${user_id eq'tjoeun' && user_pw eq number}" var="resultstr">
		tjoeun님, 하이룽~<br>
	</c:if>
	<c:if test="${ not resultstr }">
		아이디/비번을 확인하세요
	</c:if> --%>
