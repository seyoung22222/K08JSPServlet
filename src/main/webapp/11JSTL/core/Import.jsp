...
url속성에 절대경로로 지정할 경우 컨텍스트 루트명은 포함하지 않는다.
 -->
	...
	<!-- 
	import태그의 하위태그로 param을 사용할 수 있다. 지정된 페이지로 
	파라미터를 전달한다.
	 -->
	
	<!-- 
	아래와 같이 이미지를 삽입할때는 주로 상대결오를 사용하는것이 좋다.
	만약 절대경로를 사용해야 한다면 하드코딩을 하는것 보다는 request
	내장객체에서 제공하는 메서드를 사용하는 것이 좋다.
	웹프로그래밍은 웹서버에 배포하는것이 목적이므로 서버의 환경이 
	달라지면 경로도 수정해야 하므로 이를 최소화 할 수 있도록 개발하는것이 
	좋다.
	...
	<!-- 앞에서 선언한 import태그의 var속성의 변수를 통해 문서의 
	내용을 출력한다. -->
	<h4>다른 문서 삽입하기</h4>
	${contents }
